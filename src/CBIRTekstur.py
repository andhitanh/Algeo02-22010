...
class Tekstur:
    def CBIRTekstur(image,path):
        #Cek ukuran gambar, resize jika terlalu besar
        image=Tekstur.kompres(image,path)
        ...
        for i in range(0,row):
            for j in range(0,col):
                ...
                el+=1
                if (el%1000==0):
                    print(el*100/(row*col),"%")
        test = Image.fromarray(greyMat)
        test.show("pleaseWork")
    ...
    def kompres(image,path):
            while (os.path.getsize(path)>100000):
